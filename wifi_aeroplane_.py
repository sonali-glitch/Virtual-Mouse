...
def toggle_airplane_mode(state):
    """Enable or disable Airplane Mode using Windows commands."""
    if state == "on":
        subprocess.run("powershell -command \"(New-Object -ComObject WScript.Shell).SendKeys('^%{a}')\"", shell=True)
...
while cap.isOpened():
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            elif thumb_tip > index_tip:
                toggle_wifi("off")

            # Gesture: Two Fingers Up → Toggle Airplane Mode
            elif index_tip < thumb_tip and middle_tip < thumb_tip:
                toggle_airplane_mode("on")

    # Show Video Feed
    cv2.imshow("Hand Gesture Airplane Mode Control", frame)
